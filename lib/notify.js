...
;(function () {
  ...
  var pending = null
  var cardSeq = 0
  /** 上次上报的当前会话 id，用来发现「用户切了会话」。 */
  var lastReportedSession = ''
  ...
  style.textContent = [
    ...
    // 「等你回答」是**待办**，不是「刚刚发生了什么」的播报，所以给它独立颜色
    // （蓝）+ 左侧色条，让人一眼分得清「要我动手」和「已经做完了」。
    '.dshtn-card[data-kind="question"] .dshtn-dot{background:var(--dsw-static-blue-500,#3b82f6)}',
    '.dshtn-card[data-kind="question"]{border-left:3px solid var(--dsw-static-blue-500,#3b82f6)}',
    // 等你批准同样算待办，用琥珀色和「等你回答」区分开。
    '.dshtn-card[data-kind="approval"] .dshtn-dot{background:var(--dsw-static-amber-600,#dd8629)}',
    '.dshtn-card[data-kind="approval"]{border-left:3px solid var(--dsw-static-amber-600,#dd8629)}',
   ...
   * 这条通知是不是「在等你动手」？
   *
   * 两类：模型提问（question）、请求批准（approval）。它们的共同点是
   * **轮次被卡住等你**，所以既不自动消失，也不该被下一条播报悄悄顶掉。
   */
  function isWaiting(kind) {
    return kind === 'question' || kind === 'approval'
  }

  /**
   * 收掉「等你动手」的卡片 —— 只收**同一个会话**的。
   *
   * 为什么必须带上 sessionId：多个会话同时在跑时，「B 会话这一轮结束了」并不
   * 能说明「A 会话那个问题已经答完了」。以前不区分会话，于是 B 随便收个尾就会
   * 把 A 的待办卡片顶掉 —— 人正好在那时看屏幕，就再也看不到 A 在等他了。
   *
   * sessionId 缺失时**一张都不收**：不知道是谁的，就别动。
   */
  function dismissWaiting(sessionId) {
    if (!sessionId) return
    try {
      var cards = root.querySelectorAll('.dshtn-card[data-kind="question"],.dshtn-card[data-kind="approval"]')
      for (var i = 0; i < cards.length; i++) {
        if (cards[i].getAttribute('data-session') === sessionId) dismiss(cards[i])
      }
    } catch (err) { /* ignore */ }
  }
  ...
  function render(item) {
    ...
    // 那件事已经过去了（收到同会话的一条普通播报 = 那个轮次重新动起来了），
    // 把还在屏幕上的「等你动手」卡片收掉。留着就是在说谎：看起来还没答完。
    // 只收同会话的 —— 别的会话收尾并不能说明这个会话的问题答完了。
    if (!isWaiting(item.kind)) dismissWaiting(item.sessionId)
    ...
    // 卡片属于哪个会话。dismissWaiting 靠它只收自己那一摊。
    card.setAttribute('data-session', item.sessionId || '')
    ...
    // 「等你动手」不自动消失：它是待办，6 秒后自己溜走等于没提醒 ——
    // 人去泡杯茶回来，卡片早没了，只剩一个还在转的窗口。
    // 它只会被三种方式收掉：用户点 ×、点卡片、或**同会话**收到下一条通知。
    if (isWaiting(item.kind)) return
  ...
  /**
   * 页面当前选中的会话 id。DSH 把它存在 localStorage['dsh.sessions.current'] 里
   * （实测值形如 {"sessionId":"session-5c8bad87-…"}）。
   *
   * 拿它给宿主做「正在看的那个会话不弹通知」—— 看得到的东西不用再弹一次。
   * 取不到就返回空串，宿主把空串当「不知道」，那时**不抑制**：
   * 宁可多弹一条，也不静默吞掉一条。
   */
  function currentSessionId() {
    try {
      ...
      var id = parsed && parsed.sessionId
  ...
  document.addEventListener('visibilitychange', function () {
    ...
    // 「等你动手」不受 TTL 限制：普通通知过了 5 分钟确实过期了（人都看到了），
    // 但一个没回答的问题/没批准的请求不会因为过了 5 分钟就不用管了 —— 补上才对。
    if (document.visibilityState === 'visible' && pending) {
      if (isWaiting(pending.kind) || Date.now() - pending.ts < PENDING_TTL_MS) render(pending)
      pending = null
    }
  })
  ...
  function poll() {
    try {
      // 用户切了会话要立刻上报。否则宿主那边「正在看的会话」还停在上一个，
      // 刚切过去的这个会话反而会被弹通知 —— 而人就正看着它。
      if (currentSessionId() !== lastReportedSession) reportFocus()
      ...
      fetch(BASE + '/last.json', { cache: 'no-store' })
        ...
        .then(function (data) {
          ...
          var item = {
            ...
            // 聚合条目的 sessionId 是 null（覆盖多个会话），此时落到空串 = 不指认。
