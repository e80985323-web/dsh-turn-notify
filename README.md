...
| `osToast` | `true` | 系统通知 |
| `onlyWhenUnfocused` | `true` | 只在窗口失焦/不可见时发系统通知；改 `false` 就是每轮都发 || `suppressSubagents` | `true` | 屏蔽子代理（spawn/fork 的子会话）的轮次，只提醒主会话 |
| `minIntervalMs` | `3000` | 系统通知最小间隔，防止多会话同时收尾刷屏 |
| `durationMs` | `6000` | 卡片自动消失时间 |
...
| POST | `/dsh-turn-notify/test` | **自检**：`{"mode":"os"｜"inWindow"｜"both"}` 立刻走一遍真实派发路径 |
